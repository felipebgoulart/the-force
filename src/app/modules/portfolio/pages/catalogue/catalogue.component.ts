...
import {
  ...
  GITHUB_IMAGE_NAME,
  GITHUB_IMAGE_URL,
} from 'src/app/core/constants/strings';
...
export class CatalogueComponent implements OnInit {
  ...
  private getRepositories(): void {
    this.githubService.getRepos().subscribe((repos: IRepos[]) => {
  ...
  private getRepoLanguages(): void {
    this.repositories = this.repositories.map((repo: IRepos) => {
      ...
      this.githubService
        .getLanguagesRepo(repo.languages_url)
      ...
      this.filteredRepositories = this.repositories.filter((repo: IRepos) => {
        ...
        Object.entries(repo.languages).forEach((entry: any[]) => {
          if (entry[0] == lang) {
            filter =
              Number.parseInt(
                ((entry[entry.length - 1] * 100) / linesCount).toPrecision(2)
              ) > 10;
          }
        });
